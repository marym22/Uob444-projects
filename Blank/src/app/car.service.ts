...
getShowRooms(): Observable<ShowRooms[]> {
  return this.afs.collection<ShowRooms>(this.showRoomsCollectionName)
    .snapshotChanges()
    .pipe(
...
getCars(showroomId: string): Observable<Cars[]> {
  return this.afs.collection<Cars>(this.carsCollectionName, ref => ref.where('showroom', '==', showroomId))
    .snapshotChanges()
    .pipe(
      map(actions => actions.map(a => {
...
removeFromFavorites(carId: string): Promise<void> {
  ...
  return this.afs.collection(this.favoritesCollectionName, ref => ref.where('carId', '==', carId).where('userId', '==', userId))
    .get()
    .pipe(
      switchMap(snapshot => snapshot.docs[0].ref.delete())
    )
    .toPromise();
}
...
requestTestDrive(carId: string, showroomId: string, date: Date){
 ...
 return this.afs.collection(this.testDriveRequestsCollectionName).add(testDriveRequest);
}
...
constructor(type: string, manufacturer: string, model: string, color: string, mileage: number, engine: string, specifications: string[], numberOfSeats: number, features: string[], price: number, image: string, showroom: number) {
